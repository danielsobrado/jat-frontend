...
export class LangGraphApiService {
  ...
  private async directGet<T = any>(path: string, params?: Record<string, any>): Promise<T> {
    ...
    const response = await this.apiClient.fetchWithTimeout(urlString, { method: 'GET' });
    if (!response.ok) {
  ...
  private async directPost<T = any>(path: string, body: any): Promise<T> {
    ...
    const response = await this.apiClient.fetchWithTimeout(fullPath, {
      method: 'POST',
      headers: {
  ...
  private async directPut<T = any>(path: string, body: any): Promise<T> {
    console.log(`[LangGraphApiService] Making direct PUT request to: ${path}`);
    const response = await this.apiClient.fetchWithTimeout(path, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  async updateGraphDefinition(graphId: string, data: UpdateGraphRequestFE): Promise<FrontendGraphDef> {
    ...
    return this.apiClient.put<FrontendGraphDef>(`${this.prefix}/graphs/${graphId}`, transformedData);
  }

  async deleteGraphDefinition(graphId: string): Promise<MessageResponseFE> {
    return this.apiClient.delete<MessageResponseFE>(`${this.prefix}/graphs/${graphId}`);
  }
  ...
  async executeGraph(graphId: string, request: ExecuteGraphRequestFE): Promise<ExecuteGraphResponseFE> {
    return this.apiClient.post<ExecuteGraphResponseFE>(`${this.prefix}/graphs/${graphId}/execute`, request);
  }
}