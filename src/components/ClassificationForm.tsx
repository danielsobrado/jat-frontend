...
export const ClassificationForm: React.FC<ClassificationFormProps> = ({
  ...
  // Helper function to extract level-specific responses from LLM response
  const extractLevelResponses = (llmResponse: string | undefined, levels: ClassificationLevel[]): {[key: string]: string} => {
    if (!llmResponse) return {};
    
    const responses: {[key: string]: string} = {};
    
    // Default - assign full response to all levels as fallback
    levels.forEach(level => {
      responses[level.code] = llmResponse;
    });
    
    try {
      // Sort levels by level number to ensure we process from highest to lowest for better extraction
      const sortedLevels = [...levels].sort((a, b) => a.levelNumber - b.levelNumber);
      
      // First pass: Try to identify clear section headers like "Segment: ..." or "Class Classification"
      let foundAnyMatch = false;
      
      // UNSPSC specific level names
      const unspscLevelNames: Record<string, string[]> = {
        "segment": ["segment"],
        "family": ["family"],
        "class": ["class"],
        "commodity": ["commodity"],
      ...
      // Common patterns in LLM responses for different levels
      sortedLevels.forEach(level => {
        // Get the level name in lowercase for matching
        const levelNameLower = level.name.toLowerCase();
        const levelNumber = level.levelNumber;
        
        // Specific patterns for different level formats
        const patterns = [
          // Pattern 1: Level name followed by classification or colon with content until next level
          new RegExp(`${level.name}\\s*(classification|:)\\s*([\\s\\S]*?)(?=(${sortedLevels.map(l => l.name).join('|')})\\s*(classification|:)|$)`, 'i'),
          
          // Pattern 2: Level number format "Level X:" or "Level X -"
          new RegExp(`Level\\s*${levelNumber}\\s*[-:]\\s*([\\s\\S]*?)(?=Level\\s*\\d+\\s*[-:]|$)`, 'i'),
          
          // Pattern 3: Level code followed by content until next level code
          new RegExp(`${level.code}\\s*[-:]\\s*([\\s\\S]*?)(?=(${sortedLevels.map(l => l.code).join('|')})\\s*[-:]|$)`, 'i'),
          
          // Pattern 4: For UNSPSC, try matching common level names (e.g., "Segment:", "Family:", etc.)
          ...(unspscLevelNames[levelNameLower] || []).map((name: string) => 
            new RegExp(`${name}\\s*[-:][\\s]*([\\s\\S]*?)(?=(segment|family|class|commodity)\\s*[-:]|$)`, 'i')
          ),
          
          // Pattern 5: Numbered sections "1. Segment", "2. Family"
          new RegExp(`\\d+\\.?\\s*${level.name}\\s*[-:]?\\s*([\\s\\S]*?)(?=\\d+\\.?\\s*(${sortedLevels.map(l => l.name).join('|')})|$)`, 'i'),
        ];
        
        // Try each pattern until we find a match
        for (const pattern of patterns) {
          const match = llmResponse.match(pattern);
          if (match && match[match.length - 1]) { // Get the last capture group which contains our content
            const content = match[match.length - 1].trim();
            if (content.length > 10) { // Ensure we have meaningful content (not just a few characters)
              responses[level.code] = content;
              foundAnyMatch = true;
              break;
            }
          }
      ...
      // Second pass: If no clear matches, try dividing the response into equal parts
      if (!foundAnyMatch && sortedLevels.length > 0) {
        const lines = llmResponse.split('\n').filter(line => line.trim().length > 0);
        
        // If we have enough lines, divide them among the levels
        if (lines.length >= sortedLevels.length) {
          const linesPerLevel = Math.floor(lines.length / sortedLevels.length);
          
          sortedLevels.forEach((level, index) => {
            const startLine = index * linesPerLevel;
            const endLine = (index === sortedLevels.length - 1) 
              ? lines.length 
              : (index + 1) * linesPerLevel;
            
            responses[level.code] = lines.slice(startLine, endLine).join('\n');
          });
        }
      }
    } catch (e) {
      console.error('Error parsing LLM response for levels:', e);
